...
Library library_empty() { return NULL; }
...
int library_isEmpty(Library lib) { return lib == library_empty(); }
...
void library_print(Library lib) {
  if (!library_isEmpty(lib)) {
    printf("[%d,%d,%d, PREV[%d, %d]]\n", head(lib).coord.x, head(lib).coord.y,
           head(lib).total, head(lib).prev.x, head(lib).prev.y);
